...
>>>>>>> f6eac7b (Additional changes for submission committed.)
@st.cache_data
def load_cbb():
...
st.write(
    "Simulate the NCAA Tournament and view predicted brackets. "
    "Adjust the number of simulations and click 'Run Simulation'."
)
...
if st.button("Run Simulation"):
    try:
        ...
        if sample_bracket is None:
            st.error(
                "Simulation failed: Bracket could not be generated. "
                "Check team names and dataset."
            )
            st.stop()
>>>>>>> f6eac7b (Additional changes for submission committed.)
        ...
        st.error("Missing required file. Check your CSV paths.")
        st.stop()

    except Exception as error:
        st.error(f"Unexpected error during simulation: {error}")
        st.stop()
...
"""
    ...
    st.success(
        f"🏆 {region_name} Champion: {region_data['REGION_CHAMP']}"
    )
...
=======
    ...
    chart_df = (
        pd.DataFrame(list(probs.items()), columns=["Team", "Probability"])
...
>>>>>>> f6eac7b (Additional changes for submission committed.)
    ...
    double_digit_seed_teams = [
        "Siena", "South Florida", "North Dakota St.", "Furman",
        "Northern Iowa", "Cal Baptist", "LIU", "High Point",
        "Hawaii", "Texas", "Kennesaw St.", "Queens",
        "Howard", "Miami OH", "Tennessee St", "Akron",
        "Hofstra", "Wright St.", "Prairie View A&M", "UMBC",
        "Troy", "McNeese St.", "VCU", "Penn",
        "Idaho", "Kennesaw St.",
    ]

    upset_df = pd.DataFrame(
        [(team, r64_win_probs.get(team, 0))
         for team in double_digit_seed_teams],
        columns=["Team", "UpsetProbability"],
    )

    upset_df = (
        upset_df.sort_values("UpsetProbability", ascending=False)
        .head(10)
        .reset_index(drop=True)
    )
